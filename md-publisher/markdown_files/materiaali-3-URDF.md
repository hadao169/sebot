...
The URDF (Unified Robot Description Format) file is an XML-based file format that contains a structural and kinematic description of a robot. It defines, for example, the robot's bodies, joints, materials, and dimensions.

![turning radius](kuvat/kinematic.png)

The model consists of joints and links. Joints can be either fixed or movable (fixed links such as sensors and movable links such as an arm or wheels). When, for example, a LIDAR and a 3D camera are added to the robot, point clouds and other sensor data are brought into a common coordinate system, so that they describe the same environment.

The URDF model can be used to visualize and simulate the robot's structure and movement in tools such as RViz and Gazebo.
...
Create directories for settings and launch files.
...
Compile the project, and since we have added a new package, it is important to run the source command again. After this, we can launch the robot's URDF model and ensure its functionality.
