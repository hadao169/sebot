...
Finally, we will create a webpage that facilitates testing the ROS 2 system and sending commands. The page clearly displays the encoder values and current speed in the top row. In addition, the user can enter desired values into the command fields and execute various commands easily with buttons.
...
**RoslibJS** is a JavaScript library that simplifies robot-related programming and communication through webpages or web applications. It enables, for example, robot control, sensor data reading, and robot state management easily with JavaScript. It is often used in web-based robot projects and IoT applications that require real-time connection and control of robot functions.
...
# Set ROSbridge to start at boot
